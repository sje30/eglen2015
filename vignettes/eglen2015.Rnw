...
As can be seen in Figure XXX, retinal mosaics normally show evidence
of spatial regularity over small distances, in this case up to around
XXX um.  One common approach used in spatial statistics to compare
model and data is to compare their K or L functions (Figure XXXa).
Informally, if the L function for the data falls within the envelope
of the L functions from the model, then say that the model fits the
data.  More formally, empirical p values can be computed to compare
model and data \citep{Diggle1986-df}.
...
[Wassle1978] to those based on Voronoi diagrams \citep{Keeley2014-dn}.
...
specifically for the analysis of retinal mosaics \citep{Rodieck1991}.
...
TODO: Explain exclusion zone, and its biological basis.
...
labelling'' hypothesis of Diggle (1986).  Figure~\ref{fig:fthmaps}
...
with random labelling.  (The experimental data are shown as example 5
in Figure~\ref{fig:fthmaps}; the other examples in
Figure~\ref{fig:fthmaps} are five examples of random relabelling of
the data.)  This is confirmed by calculating the L\ped{12} function
for the experimental data and 99 realisations of random labelling.
The L function for the data falls within the envelope formed by the 99
...
\begin{figure}[h]
  ...
  \caption{L\ped{12} function for dopaminergic amacrine data set shown
    in Figure~\ref{fig:fthmaps}.  The L function for the observed data
...
I thank Professors Heinz W\"{a}ssle, David Marshak and Abbie Hughes
for providing the retinal mosaics included in this paper.  DOCKER?
