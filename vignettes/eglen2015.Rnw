...
\begin{abstract}
  ...
  of each other. (2) Cells of one type are randomly assigned one of
  two labels to create two related populations. (3) Interactions
  between cells of different type generate functional dependencies.  I
  show briefly how spatial statistic techniques can be applied to
  investigate the nature of spatial interactions among different cell
  types.
  %% 
...
\citep[given by retinal thickness, about .2\,mm;][]{Ferguson2013-ks}
...
mosaics is therefore helpful to understanding how to classify neuronal
types.
...
the spatial relationships between cells of different types.  In
...
in mouse retina \cite{Sumbul2014-vm} these steric hindrance effects
...
hindrance effects \citep{Eglen2006-nj}.  To test for functional
independence, we examined in detail the on- and off-centre beta RGCs
in cat \citep{Eglen2006-nj}.  We extended the exclusion zone model
presented earlier such that were effectively three \dmin exclusion zones
in the model, termed a pairwise interaction point process model
\citep{Ripley1976-ic,Ripley1977-av}.  The first exclusion zone, \dminoo,
was for modelling homotypic interactions between pairs of on-centre
neurons, and likewise a second exclusion zone, \dminff, for homotypic
interactions between off-centre neurons.  The third \dmin
exclusion zone, \dminof, was used for for modelling the heterotypic
interactions between an on-centre and an off-centre neuron.  The
effective values of \dminoo and \dminff for homotypic interactions was
quite large, around 50--100\,\um .  By contrast, we set \dminof to the
relatively small value of 18\,\um\ --- the average diameter of RGC
soma.  An example of the model output using this bivariate model is
shown in Figure~\ref{w81}B.  This model was judged to be a good fit to
the data when comparing various spatial statistics of the observed
data and model simulations \citep{Eglen2006-nj}.  We therefore
concluded that the spatial positioning of on- and off-centre beta RGCs
...
of neuron.  Figure~\ref{fig:fthmaps} demonstrates the random labelling
hypothesis, showing one real field, along with five random
realisations where for each cell, its location is preserved, but the
type of the cell (GCL versus INL) is randomly switched, subject to
preserving the overall ratio of cells in each sub-type.
...
which in 2014 was the most popular tool in statistics in 2014
\citep{Tippmann2015-sv}.  The packaging system in R blends easily with
the reproducible research document since it allows the developer to
...
different environments may well produce different results.  To help
solve this last problem, the Docker system provides a convenient way
for the entire computing environment relating to a project to be
efficiently captured and then run.  Docker runs on all major operating
systems and provides lightweight linux virtual machines and hence a
convenient way of getting software to run on different platforms.  For
example, once installed, the entire environment required to generate
this paper can be downloaded and initialised using the command:
...
time it is run, the user can then open a web browser to run R locally
...
Outside of the sciences, the journal \textit{Quarterly Journal of
  Political Science} since 2005 has required all empirical papers to
be reproducible.  
%% See
