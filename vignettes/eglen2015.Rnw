...
<<setup-knitr,eval=TRUE,echo=FALSE,results='hide'>>=
require(knitr)
require(xtable)
require(splancs)
require(sjedmin)
...
\begin{figure}
  ...
  \caption{Example retinal mosaic.  Beta on-cells from Wassle
    1981.  On the left is the observed map, and the right is an
    example univariate simulation with matching field and density of
    points.  Scale bar is 100\,\um; soma are drawn to scale with a
    radius of 8\,\um.
    \label{fig:univ}}
\end{figure}
...
Extends beyond N=2 types \citep{Kram2010-bb6}.
...
found in two separate layers of the retina \citep{Eglen2003-dc}.  It
was unclear whether these cells in the two layers form two distinct
types, or belong to one type of neuron.  One hypothesis tested was
that simply they both belong to the same type, but that they randomly
assigned different layers within the retina.  This corresponds to the
``random labelling'' hypothesis of Diggle (1986).
Figure~\ref{fig:fthmaps} demonstrates this hypothesis, showing one
real field, along with five random realisations where the type of the
cell is randomly switched, subject to preserving the overall numbers
of cells in each sub-type.

\textbf{TODO scalebar for plot.}
...
%% included in the following pdf.  This is useful when
%% you wish to fine-tune the pdf() dimensions.
<<plot-bivariate-fthmaps,echo=FALSE>>=
@
...
\begin{figure}
  ...
  \caption{Dopaminergic amacrine neurons in ferret, showing one field
    ...
    experimental data? Scale bar is XXX\,\um.
    \label{fig:fthmaps}}
\end{figure}

\begin{figure}[h]
  \centering
  <<fth-k12,echo=FALSE,fig.width=6,fig.height=4>>=
@ 
\caption{L function for dopaminergic amacrine data set.  TODO: asp=1 needed }
\label{fig:fthk12}
\end{figure}

Figure~\ref{fig:fthk12} shows the goodness of fit under the hypothesis
of random labelling.  TODO: explain ranking.
...
systematic study examining ten pairs from five types of retinal neuron
failed to reveal any cross-correlation [Rockland].  CAT.  The most
prominent example found to date has been in macaque, comparing the
spatial location of blue cone photoreceptors (BC) and their synaptic
targets in an adjacent layer, the blue cone bipolar cells (BCBP).  An
example of such a mosaic is shown in Figure~\ref{fig:BCBP}.


\citep{Kouyama1992-cf3,Kouyama1997-wm}
...
\begin{figure}[h]
  ...
  \caption{BCBP / BC data set (left) and L function (right) for the
    five fields? TODO: complete figure}
\label{fig:BCBP}
\end{figure}
