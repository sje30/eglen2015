...
results (typically in the form of figures or graphs) that have been
published before in other articles.  For computational work, this
typically means include the results of previous modelling or analysis.
I have deliberately taken a different approach in this article: all of
the results presented here have been generated afresh, albeit based on
my previous work, and new figures generated for the results.  The one
exception to this is Figure~\ref{fig:layers}, which is a schematic of
retinal layers, rather than showing any results.  Furthermore, all the
code and data required for others to generate new figures are
available as part of an R package.  This package is publicly available
at \url{http://github.com/sje30/eglen2015}; see that URL for full
