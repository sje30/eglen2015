...
the chunk and filename to generate each figure.
...
overcome, mostly when dealing with large data sets or with intensive
computation.  Another technical problem is how to ensure that another
user has a compatible environment for running your software.  Running
the same code in different environments may well produce different
results.  To help solve this last problem, the Docker system provides
a convenient way for the entire computing environment relating to a
...
different platforms.  For example, once installed, the entire
...
neuroscience.  Most recently, since October 2014, Nature journals
require a statement whether (and where) the code underlying research
results is available \citep{noauthor_2014-mv}.
